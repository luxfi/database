...
func TestIteratorClosed(t *testing.T, db db.Database) {
...

	{
		...
		require.Error(iterErr)
		require.Contains(iterErr.Error(), "closed")
	}
...
// 	testClear(t, db, func(db db.Database) error {
// 		return db.AtomicClear(db, db)
// 	})
// }
...
// 	testClear(t, db, func(db db.Database) error {
// 		return db.Clear(db, math.MaxInt)
// 	})
// }
...
// func testClear(t *testing.T, db db.Database, clearF func(db.Database) error) {
// 	require := require.New(t)
...
// 	testClearPrefix(t, db, func(db db.Database, prefix []byte) error {
// 		return db.AtomicClearPrefix(db, db, prefix)
// 	})
// }
...
// 	testClearPrefix(t, db, func(db db.Database, prefix []byte) error {
// 		return db.ClearPrefix(db, prefix, math.MaxInt)
// 	})
// }

// testClearPrefix tests to make sure prefix deletion works as expected.
// Commented out as it uses Count which is not part of the standard Database interface
// func testClearPrefix(t *testing.T, db db.Database, clearF func(db.Database, []byte) error) {
// 	require := require.New(t)
...
func FuzzNewIteratorWithPrefix(f *testing.F, db db.Database) {
	...
	f.Fuzz(func(
		...
		// Commented out as AtomicClear is not part of the standard Database interface
		// require.NoError(db.AtomicClear(db, db))
	})
}
